...
class Unisens(Entry):
    ...
    def __init__(self, folder, makenew=False, comment:str='', duration:int=0, 
        ...
        if os.path.isfile(self._file) and not makenew:
            ...
            self.set_attr('duration', duration)
            self.set_attr('measurementId', measurementId)
            self.set_attr('timestampStart', timestampStart)
            self.set_attr('version', '2.0')
            # self.set_attr('xsi:schemaLocation',"http://www.unisens.org/unisens2.0"+\
            #               " http://www.unisens.org/unisens2.0/unisens.xsd")
            # self.set_attr('xmlns',"http://www.unisens.org/unisens2.0")
            # self.set_attr('xmlns:xsi', "http://www.w3.org/2001/XMLSchema-instance")
    ...
    def unpack_element(self, element:( Element, ET)) -> Entry:
        ...
        if entryType == 'customAttributes':
            ...
            name = element.tag
            entry = MiscEntry(name=name, attrib=attrib, folder=self._folder)
        else:
